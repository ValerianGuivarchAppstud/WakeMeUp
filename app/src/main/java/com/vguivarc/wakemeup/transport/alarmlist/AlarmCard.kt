...
fun AlarmCard(viewModel: AlarmListViewModel?, alarm: Alarm, editing: Boolean) {
    ConstraintLayout(
        ...
        val (delete,divider, itemAlarmDaysList, editAlarmArrow, itemAlarmTime, itemAlarmDays, itemAlarmSwitch, editAlarmRepeatCheck) = createRefs()
        ...
        if(editing) {
            Row(
                modifier = Modifier.constrainAs(editAlarmRepeatCheck) {
                    top.linkTo(itemAlarmTime.bottom, margin = 0.dp)
                    start.linkTo(parent.start, margin = 16.dp)
                }
            ) {
                Checkbox(
                    checked = alarm.isRepeated,
                    onCheckedChange = { viewModel?.actionRepeatSelected(alarm, !alarm.isRepeated) }
                )
                Text(text = "Répéter", modifier = Modifier.padding(16.dp, 0.dp))
            }
            if(!alarm.isRepeated) {
                Row(
                    ...
                    modifier = Modifier
                        ...
                        .constrainAs(itemAlarmDaysList) {
                            top.linkTo(editAlarmRepeatCheck.bottom, margin = 16.dp)
                        }
                ) {
