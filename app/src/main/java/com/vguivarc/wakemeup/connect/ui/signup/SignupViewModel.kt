...
class SignupViewModel(val repo : Repository) : ViewModel() {

    private val _signupForm = MutableLiveData<SignupFormState>()
    val signupFormState: LiveData<SignupFormState> = _signupForm

    private val _signupResultState = MediatorLiveData<ConnectResult>()
    fun getSignupResultLiveData(): LiveData<ConnectResult> = _signupResultState

    init {
        _signupResultState.addSource(repo.signupResult) { signupResult ->
            _signupResultState.value = signupResult
        }
    }
    ...
    fun signup(
        ...
        repo.createUserWithEmailAndPassword(username, mail, password)
    }
