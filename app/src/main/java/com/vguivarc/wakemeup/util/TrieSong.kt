...
fun trieAlphabetiqueFavoris(favorisVideoListe : MutableList<Favori>, context : Context){
    favorisVideoListe.sortWith(object: Comparator<Favori>{
        override fun compare(songH1 : Favori, songH2 : Favori) : Int =
            songH1.idSong.title.compareTo(songH2.idSong.title)
    })
}

fun trieDateAjoutFavoris(favorisVideoListe : MutableList<Favori>, context : Context){
    favorisVideoListe.sortWith(object: Comparator<Favori>{
        override fun compare(songH1 : Favori, songH2 : Favori) : Int =
            songH2.index.compareTo(songH1.index)
    })
