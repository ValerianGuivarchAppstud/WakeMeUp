...
class MusiquesAttenteFragment : Fragment(), SonnerieAttenteAdapter.RecyclerItemClickListener {
    ...
    private val sonnerieAttenteList = mutableListOf<Sonnerie>()
    ...
    private fun updateAttenteListe(state: Map<String, Sonnerie>) {
            sonnerieAttenteList.clear()
            sonnerieAttenteList.addAll(state.values)
            //todo tri par date
        if(sonnerieAttenteList.size>0) {
            sonnerieAttenteList.sortWith(compareBy { v -> v.dateEnvoie })
    ...
    override fun onClickSonnerieListener(sonnerie: Sonnerie, position: Int) {
