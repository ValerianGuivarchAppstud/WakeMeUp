...
import com.vguivarc.wakemeup.notification.*
...
class ListFriendToSendMusicActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        list_partage_ami_valider.setOnClickListener {
            ...
            for (index in listeSelection) {
                ...
                val pushReveil = AppWakeUp.repository.database.reference.child("Reveils").push()
                pushReveil.setValue(reveilMusic).addOnSuccessListener {
                    AppWakeUp.repository.database.reference.child("Users")
                        .child(AppWakeUp.auth.currentUser!!.uid).child("sentMusic").push()
                        .setValue(pushReveil.key)
                    AppWakeUp.repository.database.reference.child("Users").child(AppWakeUp.listeAmis[index].id)
                ...
                AppWakeUp.repository.database.getReference("Users").child(AppWakeUp.listeAmis[index].id)
    ...
    private fun sendNotification(receiver: String, username: String, song: String) {
        AppWakeUp.repository.database.getReference("Tokens").orderByKey().equalTo(receiver)
            .addValueEventListener(
                object : ValueEventListener {
                    override fun onDataChange(dataSnapshot: DataSnapshot) {
    ...
    private fun updateToken(token: String, fuser: String) {
        ...
        AppWakeUp.repository.database.getReference("Tokens").child(fuser).setValue(token1)
    }
}
